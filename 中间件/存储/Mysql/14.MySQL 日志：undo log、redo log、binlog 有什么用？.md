...
下图是一条查询语句经历的过程，这属于「读」一条记录的过程，如下图：
...
> 预写式日志（**预写式日志（WAL**）是关系数据库系统中用于提供原子性和持久性的技术。所有修改在提交前都写入log文件，包括redo和undo信息
...
> 数据到文件中时，其实数据不会马上写入磁盘，而是会经过层层缓存。这些缓存的存在是为了加速读写操作，因为直接读写磁盘效率是很低的。
