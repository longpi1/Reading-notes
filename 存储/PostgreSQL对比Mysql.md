...
**1.PostgresSQL对比Mysql的优点？**

- **处理复杂查询时效率更高**，PG在查询阶段有一个重写的步骤，会按照[规则系统](https://www.postgresql.org/docs/current/static/rules.html)中存在的规则，对查询树进行改写，优化复杂的查询树。

- **对索引的支持更强**；PostgreSQL 的可以使用函数和条件索引，这使得PostgreSQL数据库的调优非常灵活，mysql就没有这个功能，条件索引在web应用中很重要。对于索引类型：

  - MySQL：取决于存储引擎。MyISAM：BTREE，InnoDB：BTREE。
  - PostgreSQL：支持 B-树、哈希、R-树和 Gist 索引等。

  InnoDB的表和索引都是按相同的方式存储。也就是说表都是索引组织表。这一般要求主键不能太长而且插入时的主键最好是按顺序递增，否则对性能有很大影响。PostgreSQL使用堆表不存在这个问题。
...
- **对于字符支持更好一些，时间精度更高**
  ...
  例如MySQL对于时间、日期、间隔等时间类型没有秒以下级别的存储类型，而PostgreSQL可以精确到秒以下。MySQL 里需要 utf8mb4 才能显示 emoji 的坑, PostgreSQL 没这个坑。

- **存储方式支持更大的数据量**

  PostgreSQL主表采用堆表存放，MySQL采用索引组织表，能够支持比MySQL更大的数据量。

- **主备复制更快**，PG主备复制用的是物理复制，而Mysql则是通过binlog
