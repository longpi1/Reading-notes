...
**IPC**全称Interprocess Communication，指进程间协作的各种方法，当然包括共享内存，信号量或Socket等。
...
和传统消息队列类似，但是在内核实现的。相比于 FIFO，消息队列具有以下优点：
...
信号量本质上是一个整型计数器，用于为多个进程提供对共享数据对象的访问。**调用`wait`时计数减一，减到零开始阻塞进程，从而达到进程、线程间协作的作用。**
